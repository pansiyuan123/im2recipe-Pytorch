...
class ingRNN(nn.Module):
    def __init__(self):
        ...
        model = word2vec.load(opts.ingrW2V)
        _ = model.vocab
        vec = model.vectors
        self.embs = nn.Embedding(vec.shape[0], opts.ingrW2VDim, padding_idx=0)  # not sure about the padding idx
        self.embs.weight.data.copy_(torch.Tensor(vec))
